...
def get_payload(training_id, model, osm_user):
...
class FeedbackAOITest(APILiveServerTestCase):
    ...
    def test_feedback_aoi_create_and_delete(self):
        payload = get_payload(1, self.model, self.osm_user)

        res = self.client.post(f"{API_BASE}/feedback-aoi/", json.dumps(payload), headers=json_type_header)
    ...
    def test_feedback_aoi_create_and_gpx_read(self):
        payload = get_payload(2, self.model, self.osm_user)
    ...
    def test_feedback_aoi_create_and_list(self):
        payload = get_payload(3, self.model, self.osm_user)
    ...
    def test_feedback_aoi_create_and_partial_update(self):
        payload_one = get_payload(4, self.model, self.osm_user)

        res = self.client.post(f"{API_BASE}/feedback-aoi/", json.dumps(payload_one), headers=json_type_header)
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        id = res.json()['id']

        payload_two = payload_one
        payload_two['source_imagery'] = "http://example23323.com"
        payload_two['geom']['coordinates'][0][2] = [32.588869114643053, 0.348171660921362]

        res = self.client.patch(f"{API_BASE}/feedback-aoi/{id}", json.dumps(payload_two), headers=json_type_header)
        self.assertEqual(res.status_code, status.HTTP_200_OK)


    def test_feedback_aoi_create_and_read(self):
        payload = get_payload(5, self.model, self.osm_user)
