...
def get_payload(feedback_aoi_id, model, osm_user):
...
class FeedbackLabelTest(APILiveServerTestCase):
    ...
    def test_feedback_label_create_and_delete(self):
        ...
        res = self.client.post(f"{API_BASE}/feedback-label/", json.dumps(payload), headers=json_type_header)
    ...
    def test_feedback_label_create_and_partial_update(self):
        ...
        payload_one = get_payload(3, self.model, self.osm_user)

        res = self.client.post(f"{API_BASE}/feedback-label/", json.dumps(payload_one), headers=json_type_header)
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        id = res.json()['id']
        ...
        res = self.client.patch(f"{API_BASE}/feedback-label/{id}", json.dumps(payload_two), headers=json_type_header)
        self.assertEqual(res.status_code, status.HTTP_200_OK)


    def test_feedback_label_create_and_read(self):
      
        payload = get_payload(4, self.model, self.osm_user)
